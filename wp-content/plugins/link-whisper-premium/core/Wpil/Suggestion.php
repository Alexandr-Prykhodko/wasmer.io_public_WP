...
class Wpil_Suggestion
{
    ...
    public static function ajax_get_post_suggestions(){
        ...
        if(isset($_POST['type']) && 'outbound_suggestions' === $_POST['type']){
            ...
            // check to make sure that the AI process has gotten around to this post
            $is_ai_processed = !empty(Wpil_AI::get_calculated_embedding_data($post->id, $post->type));

            // if we're doing suggestions with the AI method and we've processed the post
            if($ai_powered && $is_ai_processed && empty($suggested_post_ids)){
                ...
                $suggested_posts = self::getAiInboundSuggestedPosts($excluded_posts);
                $suggested_terms = self::get_ai_inbound_suggested_terms($post);
            ...
            // get all the suggested posts for linking TO this post using the original keyword method
            if(empty($suggested_post_ids)/* && !$ai_powered*/){ //TODO: Think about later and come up with a fallback option for when AI's not available
                ...
                $relatedness_threshold = 0.40; //Wpil_Settings::get_suggestion_filter('ai_relatedness_threshold'); // TODO: revisit later. For the time being, we'll just set a short bar to jump over so we can remove the most unrelated posts from a topic perspective
            ...
            if($ai_powered && (!isset($_REQUEST['keywords']) || empty($_REQUEST['keywords']))){
                $keywords = array(0 => '');
            }
                ...
                if($ai_powered){
                    ...
                    foreach ($keywords as $ind => $keyword) {
                        ...
                        if(!in_array($post_id, $ignore_posts)){
                            ...
                            if (!empty($keyword)) {
                                $suggestions = self::getAiPostSuggestions($links_post, $post, false, $keyword, $count, $key);
                            } else {
                                $suggestions = self::getAiPostSuggestions($links_post, $post, false, null, $count, $key);
                            }
                            ...
                            // add any generated suggestions to our list
            ...
            $finish = false;
            $max_suggestions = Wpil_Settings::get_max_suggestion_count();
            if($max_suggestions > 0 && count($stored_phrases) > $max_suggestions){
                $finish = true;
            }
    ...
    /**
     * Gets the suggestions AI-Powered inbound suggestions for posts.
     * Is cron powered to allow large capacity without requiring the user to keep a tab open.
     * Also doubles as Outbound Suggestion preloader! So once the suggestions are scanned, we can just zip them from the db!
     **/
    public static function cron_get_ai_inbound_suggestions(){
        // IF:
        if( 
            Wpil_Settings::disable_ai_suggestions_cron_task() || // the user disabled the task
            !Wpil_Settings::get_use_ai_suggestions() || // doesn't want to do AI suggestions
            !empty(get_option('wpil_oai_insufficient_quota_error', '0')) ||// or ran out of money
            empty(Wpil_Settings::getOpenAIKey()) // or there's no api key
            )
        {
            // exist
            return;
        ...
        // set the flag so that we know that we're downloading data
        ...
        // get the list of processable ids
        $ids = Wpil_Settings::get_ai_inbound_suggestion_ids();
        ...
        foreach($ids as $post_id) {
            ...
            if(empty($post->getContent())){
                ...
                Wpil_Settings::update_ai_inbound_suggestion_ids($post->get_pid());
                // and skip to the next post
                continue;
            }
            ...
            if(!$calculated && empty($embedding_data)){
                ...
                if(!empty($post_data)){
                    // save the data
                    Wpil_AI::save_single_post_embedding_data($post, $post_data);
                    // and update the embedding data flag
                    $embedding_data = true;
                }
            }
            ...
            // if the AI calculations haven't completed
            if(!$calculated && !empty($embedding_data)){
                while(!Wpil_Base::overTimeLimit(5, 35) && !Wpil_AI::has_calculated_phrase_embeddings($post)){
                    // run the calculations
                    Wpil_AI::stepped_calculate_phrase_embeddings($post, true); // TODO: Handle cases where the result is empty
                }
                // and break out of the loop so we can start processing the suggestions with a full timeout
                break;
            }
            ...
            if(empty($post_data)){
                // todo mark the post as completed in one of the processing tables
                // mark the post as complete
                Wpil_Settings::update_ai_inbound_suggestion_ids($post->get_pid());
                break;
            }
            ...
            foreach($post_data['calculation'] as $sentence => &$relations){
                foreach($relations as $pid => $score){
                    if($score < 0.50 /*self::$ai_suggestion_threashold*/){
                        unset($relations[$pid]);
                    }
                }
            ...
            // if there are no calculations or we're not doing anchor building
            if(empty($post_data['calculation']) || Wpil_Settings::get_disable_ai_anchor_building() || true){
                // mark the post as complete
                Wpil_Settings::update_ai_inbound_suggestion_ids($post->get_pid());
                // if we have time
                if(!Wpil_Base::overTimeLimit(5, 35)){
                    // go around for another post process
                    continue;
                }else{
                    // otherwise break the loop so we can start with a fresh timer
                    break;
                }
            }
            ...
            // if we're not currently in a api cooldown
            if(empty($doing_cooldown) || $doing_cooldown < time())
            {
                $new_phrases = array();
                foreach($post_data['calculation'] as $sentence => $relations){
                    foreach($relations as $pid => $score){
                ...
                if(empty($processing) && !is_null($processing)){
                    break;
                }
