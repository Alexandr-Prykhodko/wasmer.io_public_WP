...
//            $wpdb->prefix . "wpil_ai_error_log",
     ...
     * 
     **/
    public static function create_response_format($name = 'batch_response', $data = array(), $model = '', $array_processing = false){
        ...
        foreach($data as $ind => $type){
            $properties[$ind] = array(
                'type' => $type
            );
        }

        if(empty($array_processing)){
...
/*
        // setup the instructions for Chatty
        $instructions = array(
            "You are being given a sentence that is related to a specific blog post. Your job is to determine which words from that exact sentence would make the best single concise HTML anchor text to use in a link to that post",
            "You must list all words that will be used in the anchor. You may not use any words that are not in the sentence. ALL words must be consecutive, and you must not skip over words. Do not rewrite or adjust the sentence in any way. The anchor must make sense to a human reader and be long enough convey meaning, ideally between 3 and 8 words.", // Assume that the sentence is related to the post, and try to find a basis to select words for an anchor.",
            "We are giving you the post title and a comma-separated list of keywords as an indicator of what the post's topic is. When you are done selecting the words, check your work by re-evaluating the sentence again and seeing if you come to the same conclusion. If you do not, choose the conclusion that makes the most sense. Give preference to anchors that contain one or more of the keywords.",
            "The data is being given in JSON format. The \"sentence\" value is the sentence that the you will check for words related to the post title. The \"post_title\" value is the post title that you will be finding related words for. The \"keywords\" are a list of keywords that we believe are relevant to the post's topic.",
            "Your response must be in JSON with this structure {\"words\": \"\", \"sentence\": \"\", \"title\": \"\", \"keywords\": \"\", \"match_score\": \"\", \"free_form\": \"\"}. The anchor words must be in an array indexed to \"words\" and the input sentence must be indexed to \"sentence\". The input post title must be indexed to \"title\", and the input keywords must be indexed to \"keywords\".",
            "After you have entered the 'words', 'sentence' and 'title', evaluate the anchor you have proposed, justify why the anchor is or is not related to the title or keywords in the \"free_form\" field, and rank how well it relates to the target post and it's keywords on a scale of 1 to 10. With 1 being not related at all to the target post and its keywords, and 10 being perfectly related to the target post and its keywords. You must base your assessment off of the title and the keywords only.",
        //    "If the anchor is not a good match, and can not be reasonably considered related, put a \"1\" in the \"poor_match\" index.",
        //    "If the anchor is cannot be considered topically related at all, enter \"bad-match\" in the \"words\" index, and put a \"1\" in the \"bad_match\" index.",
...
/*
        $instructions = array(
            ...
            "You must list all words that will be used in the anchor. You may not use any words that are not in the sentence. ALL words must be consecutive, and you must not skip over words. Do not rewrite or adjust the sentence in any way.",
...
*/
        ...
        $instructions = array(
            "You are being given an array of JSON objects. ",
            "Each object contains a sentence that may be related to a specific blog post. Your job is to determine which words from that exact sentence would make the best single concise HTML anchor text to use in a link to the target post. ",
            "You must list all words that will be used in the anchor. You may not use any words that are not in the sentence. ALL words must be consecutive, and you must not skip over words. Do not rewrite or adjust the sentence in any way. ",
            "You MUST TREAT EACH OBJECT INDIVIDUALLY, and you MUST NOT combine information from separate objects. All objects are separate from each other and are not related, you must not use them as context for each other. ",
            "The anchor must make sense to a human reader and be long enough convey meaning, ideally between 3 and 8 words. ",
            "Inside of each object, we are giving you the target post title and a comma-separated list of keywords as an indicator of what the target post's topic is. ",
            "When you are done selecting the words, check your work by re-evaluating the sentence again and seeing if you come to the same conclusion. If you do not, choose the conclusion that makes the most sense based on relation to the target post's title and keywords. Give preference to anchors that contain one or more of the keywords or contains multiple words from the target post's title. ",
            "The data is being given in JSON format. The \"sentence\" value is the sentence that the you will check for words related to the post title. The \"post_title\" value is the post title that you will be finding related words for. The \"keywords\" are a list of keywords that we believe are relevant to the post's topic. The \"meta_id\" is a unique identifier for the JSON object. ",
            "Your response must be an array of JSON objects, and the JSON objects must use this structure {\"meta_id\":\"\",\"words\": \"\", \"sentence\": \"\", \"title\": \"\", \"keywords\": \"\", \"match_score\": \"\", \"free_form\": \"\"}. ",
            "The anchor words must be indexed to \"words\" and the input sentence must be indexed to \"sentence\". The input post title must be indexed to \"title\", and the input keywords must be indexed to \"keywords\". The input \"meta_id\" value must be indexed to the output \"meta_id\" index. ",
            "After you have entered the 'words', 'sentence' and 'title' for an object, evaluate the sentence to determine if it is related to the object\'s the title or the object\'s keywords and write your evaluation in the object\'s \"free_form\" field, and then rank how well the object\'s sentence relates to the object's target post and the object's keywords on a scale of 1 to 10. With 1 being not related at all to the object's target post and the object's keywords, and 10 being perfectly related to the target post and its keywords. Each \"match_score\" value must be specifically and individually calculated for each object. ",
   //         "After you have calculated the \"match_score\" and have finalized the data for each object, set the \"free_form\" value to 'pizza' without changing any of the other values that you have set in the object."
...
/*
        $instructions = array(
            "You are being given an array of JSON objects. ",
            "You MUST TREAT EACH OBJECT INDIVIDUALLY, and you MUST NOT combine information from separate objects. All objects are separate from each other and are not related, you must not use them as context for each other. ",
            "Your job is to examine the content of each object individually, and explain in the object's \"free_form\" field what consecutive string of words from the object's unedited \"sentence\" value would make the best SEO optimized link to a post titled by the object's \"post_title\" value and was ranking for keywords from the object's \"keywords\" value. ",
            "After writing up your explanation, re-assess what consecutive string of words from the unedited \"sentence\" value would make the best SEO optimized link to a post titled by the \"post_title\" value and was ranking for keywords from the \"keywords\" value and create a second explanation in a field called \"free_form_2\". ",
            "After writing your second explanation, and judge which makes the most compelling case for a link and enter the best explanation in the object's \"free_form\" field along with a note which was best. ",
            "Then based on the explanation in the object's \"free_form\" field, enter the consecutive string of words in the object's \"words\" field. When you have done this, rank on a scale of 1 to 10 how related the object's \"words\" are to the object's \"post_title\" value and the object's \"keywords\" value. ", 
            "With 1 being not related at all to the object's \"post_title\" and the object's \"keywords\", and 10 being perfectly related to the object's \"post_title\" and the object's \"keywords\". Each object's \"match_score\" value must be individually calculated for each object, and be based on the object's \"post_title\" and the object's \"keywords\" only. ",
            "When you are finished, ensure that each of the JSON objects in the array you return use this structure {\"meta_id\":\"\",\"words\": \"\", \"sentence\": \"\", \"title\": \"\", \"keywords\": \"\", \"match_score\": \"\", \"free_form\": \"\"}. "
            
/*
            "Each object contains a sentence that is related to a specific blog post. Your job is to determine which words from that exact sentence would make the best single concise HTML anchor text to use in a link to the target post. ",
            "You must list all words that will be used in the anchor. You may not use any words that are not in the sentence. ALL words must be consecutive, and you must not skip over words. Do not rewrite or adjust the sentence in any way. ",
            "You MUST TREAT EACH OBJECT INDIVIDUALLY, and you MUST NOT combine information from separate objects. All objects are separate from each other and are not related, you must not use them as context for each other. ",
            "The anchor must make sense to a human reader and be long enough convey meaning, ideally between 3 and 8 words. ",
            "Inside of each object, we are giving you the target post title and a comma-separated list of keywords as an indicator of what the target post's topic is. ",
            ...
            "Your response must be an array of JSON objects, and the JSON objects must use this structure {\"meta_id\":\"\",\"words\": \"\", \"sentence\": \"\", \"title\": \"\", \"keywords\": \"\", \"match_score\": \"\", \"free_form\": \"\"}. ",
            "The anchor words must be indexed to \"words\" and the input sentence must be indexed to \"sentence\". The input post title must be indexed to \"title\", and the input keywords must be indexed to \"keywords\". The input \"meta_id\" value must be indexed to the output \"meta_id\" index. ",
            "After you have entered the 'words', 'sentence' and 'title' for an object, evaluate the anchor you have proposed, justify why the anchor is or is not related to the title or keywords in the \"free_form\" field, and rank how well it relates to the object's target post and the object's keywords on a scale of 1 to 10. With 1 being not related at all to the object's target post and the object's keywords, and 10 being perfectly related to the target post and its keywords. Each \"match_score\" value must be individually calculated for each object, and be based on the object's title and the object's keywords only. ",
   //         "After you have calculated the \"match_score\" and have finalized the data for each object, set the \"free_form\" value to 'pizza' without changing any of the other values that you have set in the object."
        *//*
        );*/
        ...
        foreach($phrase_data as $sentence => $data){ // sentence == text && target_data == array(pid, post title, keywords) 
            ...
            foreach($data as $target_data){
                ...
                $id = md5($sentence . '|||' . $target_data[0]);
                //$id = md5($sentence);

                if(isset($already_processed[$sentence_id])/* || isset($queued[$id])*/){
                    continue;
                }
...
/*
    ...
    public static function start_batch_process($data, $instructions = '', $endpoint = ''){
        if(empty($data) || empty($instructions) || empty(self::$ai)){
            return false;
        }
...
/*
    ...
    public static function live_query_ai_results($data, $instructions = '', $endpoint = ''){
        ...
        if($endpoint === 'embeddings'){
            ...
            $args = array(
                'model' => self::$model,
                'response_format' => self::create_response_format('live_download', self::create_response_indexes(self::$purpose), self::$model, (self::$purpose === 'assess-sentence-anchors')),
                'message_list' => $message_list,
                'temperature' => 0,
                //'top_p' => 0.1,
                'frequency_penalty' => 0,
...
//                    self::save_site_post_summaries($dat_object);
    ...
    public static function live_query_single_post_embedding_data($post = null){
        if(empty($post) || !is_a($post, 'Wpil_Model_Post') || empty(self::$ai)){
            return false;
        }
