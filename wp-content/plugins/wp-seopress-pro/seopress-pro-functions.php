...
if (version_compare($actual_version, '6.7', '>=') || (defined('SCRIPT_DEBUG') && SCRIPT_DEBUG === true)) {
    ...
    {
        $optionProService = seopress_pro_get_service('OptionPro');

        if ($optionProService !== null && method_exists($optionProService, 'get404Cleaning')) {
            if ($optionProService->get404Cleaning() === '1' && ! wp_next_scheduled('seopress_404_cron_cleaning')) {
        ...
        $status = false;
        if (file_exists(ABSPATH . 'robots.txt') && '1' !== seopress_pro_get_service('NoticeOption')->getNoticeRobotsTxt()) {
            $alerts_high++;
            $status = true;
            ...
            if (!empty(seopress_get_service('GoogleAnalyticsOption')->getGA4PropertId()) && !empty(seopress_get_service('GoogleAnalyticsOption')->getGA4())) {
                ...
                if (seopress_get_service('GoogleAnalyticsOption')->getGA4PropertId() === seopress_get_service('GoogleAnalyticsOption')->getGA4()) {
                    ...
                    $args[] = [
                        ...
                        'desc'   => __('To get your Google Analytics stats in dashboard, your GA4 property ID must NOT be equals to your GA4 measurement ID.', 'wp-seopress-pro'),
                        'impact' => [
                            'high' => __('High impact', 'wp-seopress-pro'),
                        ],
                        'link' => [
 ...
 * @return (string) $ip
 **/
function seopress_pro_get_ip_address() {
